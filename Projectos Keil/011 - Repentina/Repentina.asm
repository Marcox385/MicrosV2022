...
ALARM_VERIFY: INC MINS0_A
		MOV A, MINS0_A
		CJNE A, #0AH, ADD_SEG	; Decenas de segundos
		
		MOV MINS0_A, #0
		INC MINS1_A
		MOV A, MINS1_A
		CJNE A, #6H, ADD_SEG	; Minuto
		
		MOV MINS1_A, #0
		INC HORS0_A
		MOV A, HORS0_A
		CJNE A, #4H, CHECK_HOUR3 ; Hora
		
		MOV A, HORS1_A
		CJNE A, #2H, CHECK_HOUR3 ; Día
		
		MOV HORS0_A, #0
		MOV HORS1_A, #0
		SJMP ADD_SEG

CHECK_HOUR3: MOV A, HORS0_A
		CJNE A, #0AH, ADD_SEG
		MOV HORS0_A, #0
		INC HORS1_A
		SJMP ADD_SEG

INC_SEC: JB P3.2, ADD_SEG
		JNB P3.3,ADD_MIN
...
ADD_SEG: INC SEGS0
		...
		INC HORS0
		MOV A,HORS0
		CJNE A, #04D, CHECK_HOUR ; Aumentar hora
		
		MOV A,HORS1
		CJNE A,#2H, CHECK_HOUR
		MOV HORS0, #0
		MOV HORS1, #0

CHECK_HOUR: MOV A,HORS0
		CJNE A, #0AH, END_SEG
		MOV HORS0, #0
		INC HORS1
		JMP END_SEG
...
ADD_MIN: INC MINS0
		...
		INC HORS0
		MOV A,HORS0
		CJNE A, #4H, CHECK_HOUR2
		
		MOV A,HORS1
		CJNE A,#2H, CHECK_HOUR2
		
		MOV HORS0, #0
		MOV HORS1_A, #0
		SJMP ADD_SEG

CHECK_HOUR2: MOV A,HORS0
		CJNE A, #0AH,ADD_SEG
		MOV HORS0, #0
		INC HORS1
		SJMP ADD_SEG
